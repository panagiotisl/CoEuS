 ...
 */
public class DoubleCountMinSketch {
    ...
    DoubleCountMinSketch() {
    }

    public DoubleCountMinSketch(int depth, int width, int seed) {
        this.depth = depth;
        this.width = width;
        this.eps = 2.0 / width;
    ...
    public DoubleCountMinSketch(double epsOfTotalCount, double confidence, int seed) {
        // 2/w = eps ; w = 2/eps
    ...
    DoubleCountMinSketch(int depth, int width, int size, long[] hashA, double[][] table) {
        this.depth = depth;
        this.width = width;
        this.eps = 2.0 / width;
        this.confidence = 1 - 1 / Math.pow(2, depth);
    ...
    public void add(long item, double count) {
        if (count < 0) {
     ...
     */
    public double estimateCount(long item) {
     ...
     */
    public static DoubleCountMinSketch merge(DoubleCountMinSketch... estimators) throws CMSMergeException {
    	DoubleCountMinSketch merged = null;
        if (estimators != null && estimators.length > 0) {
            ...
            double[][] table = new double[depth][width];
            int size = 0;

            for (DoubleCountMinSketch estimator : estimators) {
            ...
            merged = new DoubleCountMinSketch(depth, width, size, hashA, table);
        }

        return merged;
    }

    public static byte[] serialize(DoubleCountMinSketch sketch) {
        ...
        try {
            ...
            for (int i = 0; i < sketch.depth; ++i) {
                ...
                for (int j = 0; j < sketch.width; ++j) {
                    s.writeDouble(sketch.table[i][j]);
                }
            }
    ...
    public static DoubleCountMinSketch deserialize(byte[] data) {
        ByteArrayInputStream bis = new ByteArrayInputStream(data);
        DataInputStream s = new DataInputStream(bis);
        try {
        	DoubleCountMinSketch sketch = new DoubleCountMinSketch();
            ...
            sketch.table = new double[sketch.depth][sketch.width];
            for (int i = 0; i < sketch.depth; ++i) {
